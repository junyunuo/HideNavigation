...
- (void)setupMainView
{
    ...
    if (!_pageBgImage && !_pageLabel) {
        self.bgView = [[UIImageView alloc] initWithFrame:CGRectMake(self.gq_width - 70, self.gq_height - 30, 70, 20)];
        self.bgView.image = [UIImage imageNamed:@"home_location"];

        
        self.pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(35,5, 40, 10)];
        self.pageLabel.font = [UIFont systemFontOfSize:11];
        self.pageBgImage = [[UIImageView alloc] initWithFrame:CGRectMake(10, 5, 15, 10)];
        self.pageBgImage.image = [UIImage imageNamed:@"pageImage"];
        
        self.pageLabel.textColor = [UIColor whiteColor];
        [self.bgView addSubview:self.pageBgImage];
        [self.bgView addSubview:self.pageLabel];
        [self addSubview:self.bgView];
        self.bgView.hidden = !self.showPageImage;
        self.pageLabel.hidden = YES;
        self.pageBgImage.hidden = YES;
    }
}
...
- (void)setShowPageImage:(BOOL)showPageImage{
    _showPageImage = showPageImage;
    
    _bgView.hidden = !showPageImage;
}
...
- (int)currentIndex
{
    ...
    self.pageLabel.text = [NSString stringWithFormat:@"%lu/%d",(int)index % self.imagePathsGroup.count + 1,(int)self.imagePathsGroup.count];
    return MAX(0, index);
}
...
- (void)layoutSubviews
{
    ...
    self.bgView.hidden = !_showPageImage;
    if (self.backgroundImageView) {
