...
//#import "CarRoomOrderDetailsViewController.h"
...
- (void)drawBottomItems
{
    ...
    _bottomItemsView.backgroundColor = [UIColor redColor];
...
- (void)showNextVCWithScanResult:(LBXScanResult*)strResult
{
    ...
    if(![NSString isBlankString:str]){
        ...
        if(array.count >1){
            if([[array firstObject] isEqualToString:@"shop_id"]){
                NSString* shop_id = [array lastObject];
                [self loadOrderInfo:shop_id];
                return;
            }
            [self.view makeToast:@"无法识别二维码"];
        }else{
            [self.view makeToast:@"无法识别商家二维码"];
        }
    }
...
- (void)loadOrderInfo:(NSString*)shop_id{
    
//    [self showCustomProgressView];
//    NSMutableDictionary* dict = [[NSMutableDictionary alloc] init];
//    [dict setValue:shop_id forKey:@"shop_id"];
//    [[AFNService shareInstance] postValueWithMethod:shop_infoApi andBody:dict successBlock:^(SuccessModel *successModel){
//        [self hideCustomProgressView];
//        if([successModel.code integerValue]== 1){
//            PayViewController* vc = [[PayViewController alloc] init];
//            vc.shop_id = shop_id;
//            vc.title = EncodeStringFromDic(successModel.result,@"name");
