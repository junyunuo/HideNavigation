...
#pragma mark --- 返回tableview右方 indexArray 其中#在最下方
+(NSMutableArray*)IndexArrayForSpecialCharactorInLast:(NSArray*)stringArr
{
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:stringArr isFilter:NO];
    NSMutableArray *A_Result=[NSMutableArray array];
    ...
    NSString *tempString ;
    NSString *tempSpecialCharactor = nil;
    for (NSString* object in tempArray)
    {
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        //不同
        if ([pinyin isEqualToString:@"#"]) {
            tempSpecialCharactor = pinyin;
            continue;
        }
    ...
    /// 将“#”的存储最终数组中
    if ([tempSpecialCharactor isEqualToString:@"#"]) {
        [A_Result addObject:tempSpecialCharactor];
    }
...
+ (NSMutableArray*)LetterSortArrayForSpecialCharactorInLast:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        if([pinyin isEqualToString:@"#"]){
            [specialCharactorArray addObject:string];
            continue;
        }
    ...
    if (specialCharactorArray.count) {
        [LetterResult addObject:specialCharactorArray];
    }
    return LetterResult;
}
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr isFilter:(BOOL)isFilter{
