...
- (void)setup
{
    ...
    _selecIndexArray = [[NSMutableArray alloc] init];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.selectedTags containsObject:self.tags[indexPath.item]]) {
      ...
      //  NSString* tas = self.tags[indexPath.item];
     //   [self.selectedTags addObject:tas];
        
       // [self.selectedTags addObject:];
        [self.selecIndexArray addObject:[NSString stringWithFormat:@"%ld",indexPath.row]];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.canSelectTags) {
        ...
        if ([self.selectedTags containsObject:self.tags[indexPath.item]]) {
            ...
            [self.selecIndexArray removeObject:[NSString stringWithFormat:@"%ld",indexPath.row]];
        }
        else {
            if(self.selectCount <= self.selectedTags.count){
                return;
            }else{
                cell.layer.borderColor = self.tagSelectedStrokeColor.CGColor;
                cell.backgroundColor = self.tagSelectedBackgroundColor;
