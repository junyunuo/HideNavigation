...
@interface GuidancePageViewController()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
- (UIButton *)skipButton
{
    if (!_skipButton) {
        ...
        _skipButton.frame = CGRectMake(kMainScreenOfWidth - 85, 30, 65, 30);
...
- (UIPageControl *)pageControl
{
    if (!_pageControl && _imageArray.count != 0) {
        ...
        _pageControl.frame = CGRectMake(0, kMainScreenOfHeight * 0.95, kMainScreenOfWidth, 35);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.hideButtonClickBlock = ^{
        ...
        [weakSelf restoreRootViewController:[[MainViewController alloc] init]];
    };
...
{
    return CGSizeMake(kMainScreenOfWidth, kMainScreenOfHeight);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    _collectionView.bounces = (scrollView.contentOffset.x > (_imageArray.count - 2) * kMainScreenOfWidth) ? YES : NO;
    if (scrollView.contentOffset.x >  (_imageArray.count - 1) * kMainScreenOfWidth) {
